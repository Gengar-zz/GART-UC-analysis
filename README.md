...
Exact archived formal statistical inputs for Figure 7 and exact archived mouse-level values for Figure 9A and Supplementary Figure S6 are distributed with portable reproduction scripts. Observation identifiers in the Figure 7 formal-input file are endpoint-specific and only enumerate archived formal inputs; no cross-endpoint pairing is implied. Previously published two-decimal Figure 7 panel files are retained as `DISPLAY_ONLY_ROUNDED`. Figure 9B retains the values, identifiers, and formatting of the previously locked public release; its formal numerical inference is reproducible from the locked table, while upstream historical source-version provenance remains unresolved.

## Bulk formal-input provenance

The documented ACLBI workflow retrieved GEO microarray data in MINiML format, conditionally applied log2 transformation and `preprocessCore::normalize.quantiles`, mapped probes with platform annotation, excluded probes mapping to multiple genes, used the arithmetic mean when multiple retained probes mapped to one gene, and used `limma::removeBatchEffect` when applicable (historical environment: R 4.0.3, preprocessCore 1.50.0, limma 3.44.3). GSE24287 source values were print-tip-loess-normalized log2 Cy3/Cy5 values; GSE36807 and GSE37283 source values were RMA-normalized. The archived March-2026 exports did not retain the versioned platform-annotation snapshot or the per-dataset execution log identifying which conditional normalization branch was triggered. Therefore GSE24287, GSE36807, and GSE37283 are classified as `INSPECTION_ONLY_FROZEN_INTERMEDIATE`: exact archived formal inputs are distributed and downstream statistical analyses are exactly reproducible, while only the historical run-specific upstream server metadata are unavailable. Current official GART probe candidates are documented for reference in Supplementary Table S1 and are not represented as the proven March-2026 historical probe set.

## Analysis workflow and figure reproduction

The complete input contract is provided in [`INPUT_CONTRACT_AUDIT.tsv`](INPUT_CONTRACT_AUDIT.tsv), and panel-level mappings are provided in [`PUBLIC_SOURCE_DATA_TRACEABILITY.tsv`](PUBLIC_SOURCE_DATA_TRACEABILITY.tsv).

The stage order is bulk; single-cell preprocessing; GART detection/donor models; donor-paired pseudobulk, GSEA, and modules; pseudotime; scTenifoldKnk; donor-aware communication; spatial projection; figure generation; and animal endpoint verification. Panel-level input/output mappings are in `PUBLIC_SOURCE_DATA_TRACEABILITY.tsv` and `documentation/figure_to_script_map.csv`. Large expression matrices must be downloaded from the public repositories above and placed in repository-relative external-input locations described by the scripts. Figure 7–9 numerical source files provide the archived formal inputs needed to reproduce the reported analyses; original microscopy is not distributed. Figure 7 exact formal inputs are provided in `data_public/figure_source_data/Figure7/Figure7_formal_statistical_inputs.csv`, and endpoint-specific statistical results are provided in `data_public/figure_source_data/Figure7/Fig7_statistical_summary_revised.csv`. Observation identifiers are endpoint-specific and are provided only to enumerate the archived formal statistical inputs; no cross-endpoint pairing is implied. Rounded values are retained only in files explicitly prefixed `DISPLAY_ONLY_ROUNDED` and are not used for formal statistical reproduction.
...
Only de-identified numerical human and animal source data are included. Raw human microscopy, direct clinical identifiers, internal verification workbooks, and local QA files are excluded. The archived Figure 7B formal Prism analysis object is included after a metadata/privacy screen found no direct participant identifiers or local absolute paths. Original microscopy is available from the corresponding author upon reasonable request, subject to institutional and ethical requirements.
